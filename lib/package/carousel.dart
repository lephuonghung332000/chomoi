...
class Carousel extends StatefulWidget {
  ...
  const Carousel({
    ...
    this.onImageTap,
    this.onImageChange,
    this.defaultImage = const AssetImage(AppAssets.imageDefault),
  });
  ...
  Widget build(BuildContext context) {
    final List<Widget> listImages =
        (widget.images != null && widget.images!.isNotEmpty)
                          ...
                          ? Container(
                              decoration: BoxDecoration(
                                gradient: LinearGradient(
                                  ...
                                  colors: [
                                    if (widget.overlayShadowColors != null)
                                      widget.overlayShadowColors!.withOpacity(1)
                                    else
                                      Colors.grey[800]!.withOpacity(1),
                                    if (widget.overlayShadowColors != null)
                                      widget.overlayShadowColors!.withOpacity(0)
                                    else
            ...
            : [
                if (widget.defaultImage is ImageProvider)
                  Container(
                    decoration: BoxDecoration(
                      borderRadius: widget.borderRadius
                          ? BorderRadius.all(widget.radius != null
...
class DotsIndicator extends AnimatedWidget {
  ...
  Widget _buildDot(int index) {
    ...
    final double zoom = 1.0 + (dotIncreaseSize! - 1.0) * selected;
